...
(async () => {
  const out = process.env.DIAG3_OUT;
  const base = process.env.DIAG3_BASE;
  if (!chromium) {
  ...
  try {
    ...
    fs.writeFileSync(out, JSON.stringify({ ok: false, error: String(e && e.message || e), logs: logs.slice(-120), reqFailed: reqFailed.slice(0,40) }, null, 2));
  } finally {
